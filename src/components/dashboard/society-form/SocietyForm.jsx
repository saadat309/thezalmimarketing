...
// Define Zod schema for society form validation
...
export default function SocietyForm({ initialData, onSuccess, onCancel, isDuplicating, isSubmitting }) {
  ...
  useEffect(() => {
    const fetchMaps = async () => {
      try {
        const availableMapsResponse = await fetch('/api/maps?available_for=society');
        ...
        let currentSocietyMapsData = [];
        if (initialData && initialData.map_ids && initialData.map_ids.length > 0) {
          // Fetch details for maps currently associated with this society
          const currentSocietyMapPromises = initialData.map_ids.map(mapId =>
            fetch(`/api/maps/${mapId}`).then(res => {
              if (!res.ok) throw new Error(`Failed to fetch map ${mapId}`);
              return res.json();
            })
          );
          currentSocietyMapsData = await Promise.all(currentSocietyMapPromises);
        }

        // Combine and deduplicate all maps for the MultiSelect options
        const combinedMaps = [...availableMapsData, ...currentSocietyMapsData];
              ...
              <Controller
                ...
                render={({ field }) => (
                  <MultiSelect
                    ...
                    placeholder="Select maps for this society"
                  />
                )}
              />
              {errors.map_ids && (
