...
export default function CityForm({ initialData, onSuccess, onCancel, isDuplicating }) {
  ...
  useEffect(() => {
    const fetchMaps = async () => {
      try {
        const availableMapsResponse = await fetch('/api/maps?available_for=city');
        ...
        let currentCityMapsData = [];
        if (initialData && initialData.map_ids && initialData.map_ids.length > 0) {
          // Fetch details for maps currently associated with this city
          // This assumes /api/maps/:id can fetch a single map.
          // A more efficient solution would be a bulk fetch endpoint like /api/maps?ids=1,2,3
          const currentCityMapPromises = initialData.map_ids.map(mapId =>
            fetch(`/api/maps/${mapId}`).then(res => {
              if (!res.ok) throw new Error(`Failed to fetch map ${mapId}`);
              return res.json();
            })
          );
          currentCityMapsData = await Promise.all(currentCityMapPromises);
        }

        // Combine and deduplicate all maps for the MultiSelect options
        const combinedMaps = [...availableMapsData, ...currentCityMapsData];
              ...
              <Controller
                ...
                render={({ field }) => (
                  <MultiSelect
                    ...
                    placeholder="Select maps for this city"
                  />
                )}
              />
              {errors.map_ids && (
