...
export default function ImportTransferFeesModal({ isOpen, onClose, onSuccess }) {
  ...
  const handleDownloadTemplate = () => {
    window.open('/api/calculator-transfer-fees/template', '_blank');
  };
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      const response = await apiFetch('/calculator-transfer-fees/import', {
        method: 'POST',
        body: formData,
      });
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const err = await response.json();
        throw new Error(err.error || 'Failed to import transfer fees');
      }

      const result = await response.json();
      toast.success(result.message || "Transfer fees imported successfully!");
      onSuccess?.();
      onClose();
    } catch (error) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Import Calculator Transfer Fees</DialogTitle>
          <DialogDescription>
            Upload a CSV file containing reference transfer fees. Missing cities and societies must exist in the database. Phases and blocks will be auto-created. Download the sample CSV template below.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          <div className="flex flex-wrap gap-4 items-center justify-between p-4 bg-muted/50 rounded-lg border">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Sample CSV template with realistic reference transfer fees</p>
            </div>
            <div>
                ...
                {validationResult.details.map((detail, idx) => (
                  <div key={idx} className={`p-2 rounded text-xs border ${detail.is_valid ? 'bg-green-50 border-green-200 text-green-900 dark:bg-green-950/20 dark:text-green-300' : 'bg-red-50 border-red-200 text-red-900 dark:bg-red-950/20 dark:text-red-300'}`}>
                    <div className="flex justify-between font-semibold">
                      <span>Row {detail.row}: {detail.city} &gt; {detail.society} &gt; {detail.phase} {detail.block ? `(${detail.block})` : ''} - {detail.category}</span>
