...
export default function CardSlider({
  ...
  useEffect(() => {
    if (autoScrollSpeed > 0) {
      pluginRef.current = AutoScroll({ speed: autoScrollSpeed, pauseOnHover: false, stopOnInteraction: false });
    } else {
  ...
  const normalizeCustomWidths = useCallback((arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (typeof rawW === 'string') rawW = rawW.trim().toLowerCase().replace(/px$/, '');
      const w = Number(rawW);
      const c = Number(entry.cards);
      if (!Number.isFinite(w) || w <= 0 || !Number.isFinite(c) || c <= 0) continue;
  ...
  */
  useEffect(() => {
    ...
    const compute = () => {
      ...
      const nextBucketIndex = BP_MINS.findIndex(b => b.name === currentBucket.name) + 1;
      ...
      const inBucket = normalized.filter(entry => entry.width >= bucketMin && entry.width <= bucketMax && w >= entry.width);
  ...
  useEffect(() => {
    ...
    const updateArrows = () => {
      ...
      const shouldShow = hasScroll || moreThanVisible || (loop && itemsCount > 1);
      setShowArrows(Boolean(shouldShow));
    };

    updateArrows();
    emblaApi.on('select', updateArrows).on('reInit', updateArrows).on('resize', updateArrows);
    return () => emblaApi.off('select', updateArrows).off('reInit', updateArrows).off('resize', updateArrows);
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-10">
        <div className="flex items-end justify-between mb-2">
          <div className="flex flex-col items-start mb-2 md:px-8">
            {heading && <h3 className="text-2xl font-semibold sm:font-bold">{heading}</h3>}
            {subheading && <div className="mt-1 text-sm text-muted-foreground">{subheading}</div>}
          </div>
        ...
        <div className="relative">
          <div
            ...
            ref={emblaRef}
            onMouseEnter={() => { isHoveredRef.current = true; const plugin = getPlugin(); if (plugin) doPause(); }}
            onMouseLeave={() => { isHoveredRef.current = false; settleAndResume(); }}
            onClick={() => { clickedInsideRef.current = true; const plugin = getPlugin(); if (plugin) doPause(); }}
          >
          ...
          <div className="flex items-center justify-center mt-2">
            {showViewAll && (
              <Button
                variant="ghost"
                className={"shadow-md "}
                onClick={(e) => onViewAll ? onViewAll(e) : (window.location.href = viewAllHref)}
              >
