...
import { Bed, Bath, MapPin, Maximize2, ImageOff, Map, Building } from 'lucide-react';
...
const PropertyCard = (props) => {
  ...
  const finalBadges = propertyType ? [{ label: propertyType, variant: "default" }, ...(badges || [])] : (badges || []);
  ...
  const renderPrice = () => {
      ...
      return (
        <div className="space-y-0.5">
          <div className="text-base font-bold text-amber-600">
            {formattedPrice}<span className="text-xs font-normal text-primary">/month</span>
          </div>
        </div>
      );
    }
    ...
    if (finalPriceType === 'installment') {
      const periodText = finalInstallmentPeriod ? `/${finalInstallmentPeriod}` : '/month';
      
      return (
        <div className="flex flex-row items-center gap-2 space-y-0.5 justify-between">
          <div className="text-base font-bold text-amber-600">
            {formattedPrice}<span className="text-xs font-normal text-primary">{periodText}</span>
          </div>
          {installmentDuration && (
      ...
      return (
        <div className="flex flex-row items-center gap-2 space-y-0.5 justify-between">
          ...
          <div className="text-xs font-medium text-green-600">
            Save {finalCurrency + ' ' + (originalPrice - price).toLocaleString()}
          </div>
        </div>
      );
    }
      ...
      <div className="p-3 will-change-transform">
        <div className="relative overflow-hidden rounded-lg aspect-video bg-muted will-change-transform">
                    <SmartImage
                      ...
                      className="object-cover w-full h-full transition-transform duration-500 group-hover:scale-110 will-change-transform"            errorPlaceholder={
      ...
      <CardContent className="px-3 pt-2 pb-3 space-y-2 text-left will-change-transform">
        {(title || price) && (
          <div className="space-y-1.5 text-left will-change-transform">
            ...
            {price && <div className="will-change-transform">{renderPrice()}</div>}
          </div>
        )}
          ...
          <div className="flex items-center justify-between gap-2 text-left will-change-transform">
            {city && (
              <div className="flex items-center gap-1.5 text-xs font-medium text-primary will-change-transform">
                <Building strokeWidth={1.5} className="flex-shrink-0 w-3.5 h-3.5 will-change-transform" />
            ...
            <div className="flex flex-col gap-0.5 text-primary text-xs flex-1 min-w-0 text-left will-change-transform">
              {location && (
                <div className="flex items-center gap-1.5">
                  <MapPin strokeWidth={1.5} className="w-3.5 h-3.5 flex-shrink-0 will-change-transform" />
            ...
            <div className="flex flex-wrap items-center gap-2.5 sm:gap-3 pt-1.5  text-left will-change-transform">
              {beds && beds > 0 && (
                <div className="flex items-center gap-1 text-xs text-left will-change-transform">
                  <Bed strokeWidth={1.5} className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0 will-change-transform" />
              ...
              {baths && baths > 0 && (
                <div className="flex items-center gap-1 text-xs text-left will-change-transform">
                  <Bath strokeWidth={1.5} className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0 will-change-transform" />
              ...
              {area && area > 0 && (
                <div className="flex items-center gap-1 text-xs text-left will-change-transform">
                  <Maximize2 strokeWidth={1.5} className="w-3.5 h-3.5 text-muted-foreground flex-shrink-0 will-change-transform" />
