...
function list_phases(PDO $pdo) {
    $stmt = $pdo->query("SELECT id, name, slug, created_at, updated_at FROM phases ORDER BY id DESC");
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);

    foreach ($rows as &$row) {
        $row['map_ids'] = get_maps_for_entity($pdo, 'phase', $row['id']);
        $row['property_ids'] = get_properties_for_entity($pdo, 'phase', $row['id']);
        $row['file_ids'] = get_files_for_entity($pdo, 'phase', $row['id']);
    }
    send_json($rows);
}

function get_phase(PDO $pdo, $id) {
    $stmt = $pdo->prepare("SELECT id, name, slug, created_at, updated_at FROM phases WHERE id = ?");
    $stmt->execute([$id]);
...
function create_phase(PDO $pdo) {
    ...
    $slug = generate_unique_slug($pdo, 'phases', $input['name']);

    $pdo->beginTransaction();
    try {
        $stmt = $pdo->prepare("INSERT INTO phases (name, slug) VALUES (?, ?)");
        ...
        $stmt2 = $pdo->prepare("SELECT id, name, slug, created_at, updated_at FROM phases WHERE id = ?");
...
function update_phase(PDO $pdo, $id) {
    ...
    if (isset($input['name']) && $input['name'] !== $exists['name']) {
        $slug = generate_unique_slug($pdo, 'phases', $input['name'], $id);
    }
